...
public class OOSUtil {
    ...
    public String upload(MultipartFile file) {
        String url = null;
        try {
            url = upload(defaultOssAliyunField, getKey(defaultOssAliyunField.getPrefix(), FileUtil.getSuffix(file)),
                    file.getInputStream());
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public String upload(OssAliyunField ossAliyunField, MultipartFile file) {
        String url = null;
        try {
            url = upload(ossAliyunField, getKey(ossAliyunField.getPrefix(), FileUtil.getSuffix(file)),
                    file.getInputStream());
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public String getKey(final String prefix, final String suffix) {
        ...
        //文件路径
        String path = DateUtil.format(DateTimeUtil.nowTimeStamp(), DateUtil.YYYYMMDD) + "-" + uuid;
